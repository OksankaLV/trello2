...
export const CopyMoveModal = ({ titleCard, setAction, typyAction, description }: IModal): JSX.Element => {

    const dispatch = useAppDispatch()
    
    const board = useAppSelector(state => state.boards.data)
    const status = useAppSelector(state => state.boards.status)
    //const lists = useAppSelector(state => state.trello.board)
    ...
    useEffect(() => {
        ...
        if (status === "") {
            dispatch(axiosBoard())
            console.log("axiosBoard")
        }
    }, [status, board])
    useEffect(() => {
        getBoard(boardIdAction).then((data)=>{setLists(data)})
     },[boardIdAction])
    ...
    return (
        ...
        <div className="copyCard">
            <h1> Копіювати картку </h1> 
                <textarea name="" id="" value={ title } onChange={(e)=>setTitle(e.target.value)} />
            <p>на дошку  
                    <select name="" id="" onChange={(e) => setBoardIdAction(e.target.value)}>
                        <option value="">...</option>
                  {OptionBoard}
                    </select>
            </p>
            <p>лист 
                ...
                <p>позиція
                    <input type="number" value={position} onChange={
                        (e) => setPosition(e.target.value)}/></p>
            
                <button onClick={() => handleClick(typyAction)}> {typyAction=='Copy'? 'Копіювати': 'Перемістити'} </button>
            <button onClick={()=>{setAction({copy: false, move: false})}}> Відмінити </button>
        </div>    
        </div>
    )
}